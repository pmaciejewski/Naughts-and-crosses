...
void
wczytaj (int gracz, int pole, int *p)
{
...
test (int gracz, int *p)
{
  ...
  win = ((win == 1)
	 || ((znak[0] == gracz) && (znak[4] == gracz) && (znak[8] == gracz)));
  win = ((win == 1)
	 || ((znak[2] == gracz) && (znak[4] == gracz) && (znak[6] == gracz)));
...
void
pc1 (int *p)			//zrobilem z tego voida, bo nie musi zwracac wyniku, moze go od razu w funkcji zapisac
{
  int pole;
  srand (time (NULL));
  while (time (NULL) > 0)
    {				//jak dajesz while(0), to w ogole nie wchodzi do petli
      pole = rand () % 10;
      if (*(p + pole - 1) == 0)
	{
